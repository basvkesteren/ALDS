/*
    ...
    adc1.c:
           Analog to digital converter 1 driver
...
{
    if(channel & (1<<0)) {
        /* p0.6, bits 13 and 12 to 11 */
        PINSEL0 |= (3<<12);
    }
    if(channel & (1<<1)) {
        /* p0.8, bits 17 and 16 to 11 */
        PINSEL0 |= (3<<16);
    }
    if(channel & (1<<2)) {
    ...
    if(channel & (1<<4)) {
        /* p0.13, bits 27 and 26 to 11 */
        PINSEL0 |= (3<<26);
    }
    if(channel & (1<<5)) {
        /* p0.15, bits 31 and 30 to 11 */
        PINSEL0 |= (3<<30);
    }
    if(channel & (1<<6)) {
        /* p0.21, bits 11 and 10 to 10 */
        PINSEL1 &= ~(3<<10);
        PINSEL1 |= (2<<10);
    }
    if(channel & (1<<7)) {
        /* p0.22, bits 13 and 12 to 01 */
        PINSEL1 &= ~(3<<12);
        PINSEL1 |= (1<<12);
    }
...
*/
{
    ...
    AD1CR = ADC_PDN | (speed<<8) | ((1<<channel)&0xFF);
    /* OK, start conversion */
    AD1CR |= ADC_STARTNOW;
    /* Wait until the conversion is complete */
    do {
        result = AD1DRArray[channel];
...
/*!
  ...
  \code
    if(adc1_status() & ADC_DONE0) {
        dprint("ADC1 channel 0 done\n\r");
        if(adc1_overrun(0)) {
            dprint("Overruns did occure\n\r");
        }
        dprint("result: %i\n\r",adc1_data(0));
    }
    if(adc1_status() & ADC_DONE1) {
        dprint("ADC1 channel 1 done\n\r");
        if(adc1_overrun(1)) {
            dprint("Overruns did occure\n\r");
        }
        dprint("result: %i\n\r",adc1_data(1));
...
*/
{
    /* First set channel, clock and enable AD converter. Clock results
       from dividing PCLK by (the clockvalue set here + 1) */
    AD1CR = ((edge&0x01)<<27) | ADC_PDN | ((clocks&0x7)<<17) | ADC_BURST | (speed<<8) | (channel&0xFF);
    AD1CR |= trigger;

    /* configure VIC */
    vic_setup(VIC_CH_ADC1, IRQ, PRIO_ADC1, handler);

    /* Set interrupt enable bits */
    AD1INTEN = channel&0xFF;
